...
- `app_server.version`: the CLI was found but did not return a usable `--version` result. Reinstall or update the standalone Codex CLI before relying on protocol diagnostics.
- `app_server.schema`: a warning means this CLI does not provide `codex app-server generate-json-schema`; the handshake remains authoritative. A failure means a request contract used by the bridge is incompatible and the CLI should be updated or rolled back before serving traffic.
...
- `Feishu CardKit stream unavailable` in logs: enable the `cardkit:card:write` permission for the Feishu app, publish that app version, then restart the bridge. Until then, task output remains available through regular card patches.
...
- App Server connection lost: active tasks are immediately persisted as failed and their task cards state that the bridge is restarting. The bridge exits so launchd/systemd can start a clean process; manually started services must be started again. Interrupted turns are not resumed automatically.
